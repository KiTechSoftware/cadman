...
use crate::engine::{Result,models::runtime::RunMode, system::infra::process::Process, ErrorCode};
...
pub async fn run(mode: RunMode, args: Vec<String>) -> Result<()> {
    ...
    if !proc.binary_exists() {
        return Err(ErrorCode::PodmanMissing.error().with_context("binary", "podman not found in PATH"));
    }
...
pub async fn compose(mode: RunMode, args: Vec<String>) -> Result<()> {
    ...
    if !proc.binary_exists() {
        return Err(ErrorCode::PodmanComposeMissing.error().with_context("binary", "podman-compose not found in PATH"));
    }
    proc.run_async().await?.emit()
}